...
import com.alexioschiu.waterfall.adapter.BasePageViewPagerAdapter
                ...
                homeFragment?.view?.let { v ->
                    ...
                    if (recycler != null) {
                        if (recycler.computeVerticalScrollOffset() == 0) {
                            // 页面已经在顶部，则触发刷新并滚动
                            val vm = try {
                                androidx.lifecycle.ViewModelProvider(homeFragment)[com.alexioschiu.waterfall.view_model.HomePageViewModel::class.java]
                            } catch (_: Exception) {
                                null
                            }
